...
function default_bootstrap_reuse {
cat <<EOT > bootstrap.json
[
   {
      "Path":"${S3_BUCKET}/jobserver-emr/BA/existing_build_jobserver_BA.sh",
      "Name":"Custom action"
   }
]
EOT
...
function create_cluster {
    aws emr create-cluster \
        ...
        --name ${CLUSTER_NAME} \
        --instance-groups .ile://instances.json \
        --configurations file://configurations.json \
        --region ${AWS_REGION}
}

function reuse_cluster {

    default_bootstrap_reuse
